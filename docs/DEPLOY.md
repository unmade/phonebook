...
Настоящий документ содержит краткое руководство по настройке с нуля и запуску в эксплуатацию данного телефонного справочника.
...
телефонный справочник будет готов для повседневной круглосуточной эксплуатации.

После выполнения процесса по настройке и запуску приложение телефонного справочника будет запущенно на uWSGI,
...
2. Настроить пароль пользователя `PostgreSQL`:
    ```shell
...
1. Установка "менеджера пакетов" `pip` для python3:
```shell
sudo apt-get update
sudo apt-get install python-pip3
```

2. Установка окружения `virtualenv`:
    ```shell
...
```
    ...
    После выполнения данных команд, в "домашней" папке появится директория `Env`.

    > Обычно "домашняя" папка это: /home/user.
...
5. Поместить в корень "домашней" папки проект
...
7. Создайте файл `secrets.json` по адресу `backend/phonebook/phonebook/settings/`
и внесите данные:
```json
{
    "SECRET_KEY": "*",
...
в администраторскую часть сайта.
...
3. Добавить ссылку в `sites-enabled`:
```shell
...
4. Проверить правильность настройки конфигурации nginx:
    ...
    * Testing nginx configuration                         [OK]
    ...
    В этом случае необходимо убедиться, что все этапы выполнены верно
...
4. Администраторская часть будет доступна по адресу http://SERVER_IP_ADDRESS/admin

    > В качестве логина и пароля использовать данные суперпользователя.
    > Вместо SERVER_IP_ADDRESS указать IP-адрес сервера
